...
def scrape_by_tree(math_id, insert=True):
    '''TODO: pick a root and collect data starting from that root'''
    error_count = 0
    with ThreadPoolExecutor() as executor:
        future = executor.submit(get_mathematician_info, math_id, insert)
        next_futures = []
        while True:
            if future.done():
                try:
                    next_futures += [
                        executor.submit(get_mathematician_info, int(desc[0]), insert)
